...
export default function VideoStage() {
  ...
  // 2. Find the track that is currently sharing the screen (Take the first one found)
  const screenShareTrack = useMemo(() => 
    tracks.find((t) => t.source === Track.Source.ScreenShare), 
    [tracks]
  );

  // 3. Get the LIST of active cameras (Filter out muted or null tracks)
  const activeCameraTracks = useMemo(() => {
    return tracks
      ...
      .slice(0, 2); // MAX 2 VIDEO CAM as requested
  }, [tracks]);
  ...
  return (
    <div className="flex-1 h-full bg-gray-900 relative overflow-hidden flex items-center justify-center">
      ...
      <div className="w-full h-full flex items-center justify-center p-2">
        {screenShareTrack ? (
          // Priority 1: Show screen share as the main focus
          ...
          // Priority 2: Show Camera grid (Max 2)
          <div className={clsx(
          ...
          <div className="flex flex-col items-center gap-4">
             ...
             <p className="text-gray-600 text-sm font-sans">Waiting for teacher's signal...</p>
          </div>
        )}
      </div>

      {/* --- PiP (Picture-in-Picture) in bottom corner when screen sharing --- */}
      ...
      {/* Screen sharing status notification */}
      {screenShareTrack && (
